...
static uint32_t g_IR_Times[68];  //每一个边沿到来的时间。
static uint8_t g_IR_Cnt = 0;  //目前已经收到几个边沿。
static uint32_t g_IR_LastTime = 0; //保存TIM3上一次读出来的值。
...
//68个时间->计算时间差-> 判断560 低电平0 还是1690 高电平1-> 组成32位-> 校验->得到按键
